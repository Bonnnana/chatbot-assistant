...
const defaultFavoritePrompts = [
  {
    ...
    content: 'Request a document for a certificate of enrollment.',
  },
  {
    title: '📚 Schedule a consultation',
    content: 'Schedule a consultation with Prof...',
  },
];
...
export function createFavoritesStorage(): FavoritePromptsStorage {
  return {
    ...
    removePrompt: async (id: number): Promise<void> => {
      await favoritesStorage.set(prev => {
        const updatedPrompts = prev.prompts.filter(prompt => prompt.id !== id);
        return {
          ...prev,
          prompts: updatedPrompts,
        };
